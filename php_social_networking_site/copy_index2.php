    ...
    <div class="row">
        <div class="col-lg-6">
            <div class="card mb-4">
              ...
              //                 <div class="reply"><a href="javascript:void(0)" postid="'.$data['post_id'].'" commentID="'.$data['id'].'" onclick="reply(this)">REPLY</a></div>
              //                 <div class="replies">';

              //     $sql = $link->query("SELECT replies.id, username, comment FROM replies INNER JOIN users ON replies.user_id = users.id WHERE replies.comment_id = '".$data['id']."' ORDER BY replies.id DESC LIMIT 1");
            ...
            //     $isReply = $link->real_escape_string($_POST['isReply']);
            //     $commentID = $link->real_escape_string($_POST['commentID']);
            //     $postid = $link->real_escape_string($_POST['postid']);
            
            //     if ($isReply != 'false') {
            //         $conn->query("INSERT INTO replies (comment, commentID, userID, createdOn) VALUES ('$comment', '$commentID', '".$_SESSION['user_id']."', NOW())");
            ...
            //     } else {
            //         $link->query("INSERT INTO comments (`comment_by`, `comment`,`post_id`) VALUES ('".$_SESSION['user_id']."','$comment','$postid')");
            //         $sql = $link->query("SELECT comments.id, username, comment FROM comments INNER JOIN users ON comments.comment_by = users.id ORDER BY comments.id DESC LIMIT 1");
            //     }
            ...
            // }
                ...
                if($res = get_all_friends_post($link)){
                  while ($res_data= mysqli_fetch_assoc($res)){
                    ...
                    $comment_count = comments_count($link,$postid);
                    ...
                    echo<<<product
                      ...
                      <div class="media mb-3">
                        ...
                        <div class="card-footer">
                          ...
                          <input type="text" class="maincomment-$postid" placeholder="add comment">
                          <button type="button" postid=$postid class="addcomment" isReply = "false">post</button>
                          </form>

                          </div> 
                          <div class="row replyRow-$postid" style="display:none">
                            <div class="col-md-12">
                                <textarea class="form-control" id="replyComment" placeholder="Add Public Comment" cols="30" rows="2"></textarea><br>
                                <button style="float:right" class="btn-primary btn" isreply="true" id="addReply">Add Reply</button>
                                <button style="float:right" class="btn-default btn" onclick="$('.replyRow').hide();">Close</button>
                            </div>
                          </div>

                          <strong id="numcomments">$comment_count</strong>Comments</small>
                          </div>
                ...
                <script>
                  var isReply = false, commentID = 0, max = <?php echo $comment_count ?>, postid=1;
                  $(document).ready(function(){
                    $(".addcomment, #addReply").on('click', function () {

                      
                      isreply = $(this).attr("isreply");
                      ...
                      if (isReply == 'false'){
                        comment = $(".maincomment-"+postid).val();
                      }
                  ...
                  function reply(caller){
                    ...
                    postid = $(caller).attr('postid');
                    $(".replyRow-"+postid).insertAfter($(caller));
                    $('.replyRow').show();
                  }
                </script>
              </div>
...
<!-- #chodte hai isko, jquery se comment reply system ho jyga, will see it separately after after doing framework  -->